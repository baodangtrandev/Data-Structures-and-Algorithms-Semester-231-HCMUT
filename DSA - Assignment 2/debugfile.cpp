...
class SRestaurant:public Restaurant{
	...
	void AddCustomer(Customer* cus){
		...
		if(!find(ID,areas)){
			...
			areas[areas.size()-1]->insertCustomer(cus);
			std::make_heap(areas.begin(), areas.end(), greaters());
	...
	void reHeapUp(int i){
		int p = parent(i);
		if(p < 0)return;
		if(areas[p]->count > areas[i]->count){
			swap(areas[p], areas[i]);
			reHeapUp(p);
		}else if(areas[p]->count == areas[i]->count){
			if(areas[p]->timeArea > areas[i]->timeArea){
				swap(areas[p], areas[i]);
				reHeapUp(p);
			}
		}
	}
	void reHeapDown(unsigned int i){
    unsigned int l = left(i);
    unsigned int r = right(i);
    unsigned int smallest = i;

    if((l < areas.size()) && (areas[l]->count < areas[smallest]->count)){
        smallest = l;
    }else if((l < areas.size()) && (areas[l]->count == areas[smallest]->count)){
		if(areas[l]->timeArea < areas[smallest]->timeArea){
			smallest = l;
		}
	}
	if((r < areas.size()) && (areas[r]->count < areas[smallest]->count)){
        smallest = r;
    }else if((r < areas.size()) && (areas[r]->count == areas[smallest]->count)){
		if(areas[r]->timeArea < areas[smallest]->timeArea){
			smallest = r;
		}
	}

    if(smallest != i){
        swap(areas[i], areas[smallest]);
        reHeapUp(smallest);
    }
}
	...
	void DeleteCustomer(int num){
		...
		// for (int i = 0; i < areas.size(); i++)
		// {
		// 	cout << "ID:  " << areas[i]->ID << "  COUNT: " << areas[i]->count <<"  Time: " << areas[i]->timeArea << endl;
		// }
		...
		// {
		// 	cout << "ID:  " << sortedAreas[i]->ID << "  COUNT: " << sortedAreas[i]->count <<"  Time: " << sortedAreas[i]->timeArea << endl;
		// }
...
// in ra ID của các khu vực trong min Heap và số lương khách hàng trong minHeap đó
	void printHeap(){
		cout << "---------------------------------------------------"<< endl;
		for (unsigned int i = 0; i < areas.size(); i++)
		{
...
class imp_res:Restaurant
{
	...
	public:	
		...
		void LAPSE(string name){
			...
			cout <<"[Debug] Result=" << new_customer->result << endl;
			...
			if(new_customer->result % 2 == 1){
				...
				//cout << "SRestaurant: ID "<< new_customer->result%MAXSIZE+1 <<"  Customer: " << new_customer->result <<endl;
